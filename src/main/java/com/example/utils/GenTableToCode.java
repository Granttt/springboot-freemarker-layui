...
public class GenTableToCode {
    private String packageOutPath = "com.example.domain";//指定实体生成所在包的路径
    private String authorName = "xxxxx";//作者名字
    private String tableName = "t_goods_act";//表名
    private String[] colNames; // 列名数组
    private String[] colTypes; //列名类型数组
    private int[] colSizes; //列名大小数组
    private String changeTableNameStr = "TGoodsAct";//驼峰转换后的类名
    private boolean f_util = false; // 是否需要导入包java.util.*
    private boolean f_sql = false; // 是否需要导入包java.sql.*
    private boolean f_decimal = false;//是否需要导入java.math.BigDecimal

    //数据库连接
    private static final String URL ="jdbc:mysql://localhost:3306/activities";
...
//        tableNames.add("user");
        tableNames.add("t_goods_act");//数据库真实表名
